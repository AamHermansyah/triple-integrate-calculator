...
function Documentation() {
  ...
  return (
    <div className={quicksand.className}>
      ...
      <button
        type="button"
        className="mt-4 text-sm sm:text-base py-2 px-4 rounded uppercase tracking-widest bg-sky-500"
        onClick={() => {
          setIsOpenDocumentation((prev) => !prev);
        }}
      >
        {isOpenDocumentation ? 'Tutup' : 'Baca'}
      </button>
      {isOpenDocumentation && (
        <div>
      ...
      {isOpenDocumentation && (
        <button
          ...
          className="mt-6 block mx-auto border-b-2 border-b-sky-500 px-4 py-1"
        >
          Tutup
        </button>
      )}
